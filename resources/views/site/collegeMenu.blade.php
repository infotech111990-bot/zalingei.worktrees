<div class="primary-navigation-wrapper">
    ...
    <nav class="collapse navbar-collapse bs-navbar-collapse navbar-right" role="navigation">
                <ul class="nav navbar-nav">
                    <li><a href="{{ url('/') }}/">{{Lang::get('site.universityHome')}}</a></li>
                    <li><a href="{{ url('/') }}/{{$college->slug}}">@lang('site.collegeHome',['ar'=>$college->type->titleSingle, 'en'=>$college->type->titleSingleEn])</a></li>
                    <li class="dropdown">
                        ...
                        <ul class="list-unstyled child-navigation">
                            ...
                            @if($college->type->id == 1 && $college->hasDetails('regulations'))
                                <li><a href="{{ url($college->slug.'/regulations') }}">@lang('site.regulations')</a></li>
                            @endif
                            @if($college->type->id == 1 && $college->hasDetails('programs'))
                                <li><a href="{{ url($college->slug.'/programs') }}">@lang('site.programs')</a></li>
                            @endif
                            @if($college->type->id == 1 && $college->hasDetails('calendar'))
                                <li><a href="{{ url($college->slug.'/calendar') }}">@lang('site.calendar')</a></li>
                            @endif
                            @if($college->type->id == 1 && $college->hasDetails('admission'))
                                <li><a href="{{ url($college->slug.'/admission') }}">@lang('site.admission')</a></li>
                            @endif
                        </ul>
                    </li>
                    ...
                    @if($college->announcements->count() > 0)
                        <li><a href="{{ url('/') }}/{{$college->slug}}/announcements">@lang('site.collegeAnnouncements',['ar'=>$college->type->titleSingle, 'en'=>$college->type->titleSingleEn])</a></li>
                    @endif
                    @if($college->type->display_staff == 1)
                        @if($college->staff->count() > 0)
                            <li><a href="{{ url('/') }}/{{$college->slug}}/staff">@lang('site.staff')</a></li>
                        @endif
                    @endif
                    @if($college->type->display_prof == 1)
                        @if($college->professors->count() > 0)
                            <li><a href="{{ url('/') }}/{{$college->slug}}/prof">@lang('site.professors')</a></li>
                        @endif
                    @endif
                    {{-- @if($college->type->display_students == 1)
                        <li><a href="{{ url('/') }}/{{$college->slug}}/students">@lang('site.students')</a></li>
                    @endif --}}
                        ...
                        <li class="dropdown">
                            ...
                            <ul class="list-unstyled child-navigation">
                                @foreach ($college->extraDetails as $ed)
                                    <li><a href="{{ url($college->slug.'/content/'.$ed->id) }}">@lang('site.getContent',['ar'=>$ed->title,'en'=>$ed->titleEn])</a></li>
                                @endforeach
                            </ul>
                        </li>
                    @endif
