...
<div class="container" style="padding:45px 15px">
    ...
    <div class="panel panel-default" style="border-radius:14px">
        ...
        <tr><td>{{ $grade->course->code }} — {{ $grade->course->name_en }}</td><td>{{ optional($grade->semester)->name }}</td><td>{{ $grade->total_score ?? '—' }}</td><td><strong>{{ $grade->letter_grade ?? '—' }}</strong></td><td>{{ $grade->grade_points ?? '—' }}</td></tr>
