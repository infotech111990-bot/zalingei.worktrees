...
@foreach($semesterGrades as $g)<tr><td>{{ optional($g->course)->code }}</td><td>{{ optional($g->course)->name ?: optional($g->course)->name_ar }}</td><td>{{ optional($g->course)->credit_hours }}</td><td>{{ $g->total_score ?? '—' }}</td><td>{{ $g->letter_grade ?? '—' }}</td><td>{{ $g->grade_points ?? '—' }}</td></tr>@endforeach
</tbody></table></div>
@empty
@endforelse
