								...
								<div class="row">
									...
									<div class="form-group">
										<div class="col-md-12 col-sm-12 padding-top-15">
											...
											<input type="text" name="slug" value="{{ $data->slug }}" class="form-control required {{ $errors->has('slug')? 'error' : '' }}">
											@if ($errors->has('slug'))
												<span class="help-block text-danger">
									...
									<div class="form-group">
										<div class="col-md-12 col-sm-12 padding-top-15">
											...
											<select name="colleges_type_id" class="form-control pointer required">
												<option value="0" {{ ($data->colleges_type_id == 0)? 'selected' : '' }}> نوع المرفق </option>
												@foreach (App\CollegesType::get() as $ct)
													<option value="{{ $ct->id }}" {{ ($data->colleges_type_id == $ct->id)? 'selected' : '' }}> {{ $ct->title }} </option>
												@endforeach
											</select>
										</div>
									</div>	
